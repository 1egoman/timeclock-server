...
// the base timecard that is extended whenever a new timecard is to be created.
...
// on the project that has already been paid.
function totalPaidDuration(data) {
  return data.card.reduce((acc, day) => {
    return acc + day.times.reduce((acc, time) => {
      if (day.disabled) {
        return acc + getDurationFor(day, time);
...
function getReportTemplate(filename) {
  ...
  if (match) {
  ...
  } else if (filename.startsWith("./") && !process.env.BLOCK_LOCAL_FILE_TEMPLATES) {
    // resolve relative to timecard location
    return indexCard().then((timecard_path) => {
      return fsp.readFile(path.join(path.dirname(timecard_path), filename.slice("./".length)));
    });
    ...
    // a default one
    return repo.getFileFromRepo("waltz-app", "themes", `${filename || "default"}.ejs`);
  }
}
...
function getTimecardRenderDetails(timecard, args) {
  let ejs_data = {
    timecard: timecard,
    args: Object.assign({}, timecard.args, args),

    totalTime: totalDuration(timecard),
    totalCost: timecard.totalRate || null,
    paidTime: totalPaidDuration(timecard),
    paidCost: null,
  };
  ...
  if (timecard.hourlyRate) {
    ...
    ejs_data.paidCost = Math.round(100 * (ejs_data.paidTime / 3600) * timecard.hourlyRate) / 100
  }
  return ejs_data;
}
