  ...
  if (typeof timecard === "object" && timecard.card) {
    if (timecard.card.length) {
      return timecard.card.every((date) => {
        return typeof date.date === "string" && date.times && date.times.every((time) => time.start && time.end);
      });
    }
  } else {
    return false;
...
function getReportTemplate(filename) {
  return new Promise((resolve, reject) => {
    ...
    if (match) {
      ...
      // a default one
      resolve(repo.getFileFromRepo("1egoman", "waltzmaker", `templates/${filename}.ejs`));
    }
  });
}
