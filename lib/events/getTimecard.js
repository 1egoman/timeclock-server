...
module.exports = function getTimecard(action, socket) {
  return repo.getFileFromRepo(action.user, action.repo, null, action.branch, socket.request.user)
  ...
  .then((timecard) => {
    ...
    if (err && err.message && err.message.indexOf("Not Found") >= 0) {
      throw "NO_TIMECARD_IN_REPO";
    } else {
      throw new Error(`Error fetching timecard: ${err.toString()}`);
    }
  });
}
