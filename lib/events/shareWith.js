...
module.exports = function shareWith(action, socket) {
  if (action.user && action.repo) {
    return User.findRepo(socket.request.user, action.user, action.repo)
    .then((repo) => {
        ...
        body: `
          <body class="body" bgcolor="#eee" style="background: #EEE; padding: 64px; color: #333;">
            ...
            <div style="width: 500px; margin: 64px auto; background: #FFF; padding: 20px;">
              <h1>Hello!</h1>
              <p>
                You have been invited to look at a timesheet for ${action.user}/${action.repo} on Waltz, a
                simple way for freelancers to create great looking timesheets for their clients.
              </p>
              <p style="margin-bottom: 20px;">${action.message || ""}</p>
