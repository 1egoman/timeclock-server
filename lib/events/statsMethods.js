...
function calculatePaymentStats(timecard, paymentFrequencyBreakdown) {
  // How much was I paid last month?
  // Loop through all times. If the time was in the last month, add it to the
  // total. Then, multiply the total by the hourly rate to get the final value.
  let lastMonth;
  ...
  let paymentFrequencyHours = null;
  if (timecard.card.length) {
    ...
    paymentFrequencyHours = total / totalPaidMonths;
  }
  ...
  let longestPaymentFrequencyHours = timecard.card.reduce((lastLength, day) => {
    let value = day.times.reduce((last, current) => {
  ...
  // The total time that has been worked, and the amount that has been paid.
  let timeWorked = timecard.card.reduce((acc, day) => {
    return acc + day.times.reduce((acc, time) => {
      return acc + card.getDurationFor(day, time);
    }, 0);
  }, 0);
  let timePaid = card.totalPaidDuration(timecard) * 1000;

  // also, calculate the wage for those times too
  let amountValued, amountEarned;
  if (timecard.hourlyRate) {
    amountValued = convertMillisecondsToHours(timeWorked, 3) * timecard.hourlyRate;
    amountEarned = convertMillisecondsToHours(timePaid, 3) * timecard.hourlyRate;
  }

  return {
    lastMonth,
    paymentFrequencyHours,
    longestPaymentFrequencyHours,

    timeWorked, amountValued,
    timePaid, amountEarned,
...
// Calculate the score of a user given their payment history
const TIME_WEIGHT = 1.2, PAY_WEIGHT = 1, SPEEDY_PAY_WEIGHT = 1.4;
function calculateUserScore(
  payAverage,
  longestBeforePaying,
  timeWorked,
  timePaid,
  paymentFrequencyGraph
) {
  function cast(n) {
    if (n > 1) {
      return 1;
  ...
  // the percentage of that were paid off the fastest.
  let percentPaidFast = 1, percentPaidSlow = 1;
  if (paymentFrequencyGraph && paymentFrequencyGraph.length > 0) {
    percentPaidFast = paymentFrequencyGraph[0].value;
    percentPaidSlow = _.last(paymentFrequencyGraph).value;
  }

  let part = cast(timePaid / timeWorked * TIME_WEIGHT) *
             cast(payAverage / longestBeforePaying * PAY_WEIGHT) *
             cast(percentPaidFast / percentPaidSlow * SPEEDY_PAY_WEIGHT);

  return cast(Math.log10(part * 150) / 1.5);// * (3.2 * TIME_WEIGHT * PAY_WEIGHT * SPEEDY_PAY_WEIGHT);
}
