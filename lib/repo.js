      ...
      if (err) {
        ...
        github.search.users({
          ...
          if (err) {
            ...
            let our_user = data.items.find((u) => {
              if (typeof u.login === "string") {
                return u.login.toLowerCase() === user.toLowerCase();
              } else {
                return false;
              }
            });
            if (our_user) {
              let cached_user = {
                username_lower: our_user.login.toLowerCase(),
              ...
              console.log("saving user!!!", cached_user);
...
// attempt to update the database for a repo with metadata needed right away
exports.saveRepoMetaToDatabase = function(user, whichrepo, timecard) {
  return User.getRepos(user).then((repos) => {
    let repository = repos.find((r) => {
    ...
    if (repository) {
      ...
      repository.client_name = timecard.clientName;
